...
check_ndk_binary() {
    local path
    path="$ANDROID_NDK_DIR/toolchains/llvm/prebuilt/linux-x86_64/bin/$TARGET_BIN_PREFIX$ANDROID_PLATFORM_VERSION-$1"

    if [[ ! -f $path ]]; then
        carp "$1 does not exist at $path - generating it"
        set -xe
        echo '#!/usr/bin/env bash' >"$path.tmp"
        echo "# This script is generated by Drawpile's pkg/android/build.bash" >>"$path.tmp"
        echo "# (Because this NDK is missing it, probably because it's an LTS version.)" >>"$path.tmp"
        echo 'bin_dir=`dirname "$0"`' >>"$path.tmp"
        echo 'if [ "$1" != "-cc1" ]; then' >>"$path.tmp"
        echo '    "$bin_dir/'"$1"'" --target='"$TARGET_BIN_PREFIX$ANDROID_PLATFORM_VERSION"' "$@"' >>"$path.tmp"
        echo 'else' >>"$path.tmp"
        echo '    # Target is already an argument.' >>"$path.tmp"
        echo '    "$bin_dir/'"$1"'" "$@"' >>"$path.tmp"
        echo 'fi' >>"$path.tmp"
        chmod +x "$path.tmp"
        mv "$path.tmp" "$path"
        set +xe
...
run_build_script() {
    ...
    cmake \
        ...
        -DCMAKE_ANDROID_NDK="$ANDROID_NDK_DIR" \
...
: "${BUILD_TYPE:=release}"
: "${DEPS_TYPE:=same}"
: "${QT_VERSION:=5.15.7.K4b3b2a81b7a285e5705e70d11e5535fddc89b3e1}"
...
case $ANDROID_ABI in
    'armeabi-v7a')
        TARGET_ARCH='arm32'
        TARGET_BIN_PREFIX='armv7a-linux-androideabi'
        ;;
    'arm64-v8a')
        TARGET_ARCH='arm64'
        TARGET_BIN_PREFIX='aarch64-linux-android'
        ;;
    'x86')
        TARGET_ARCH='x86'
        TARGET_BIN_PREFIX='i686-linux-android'
        ;;
    'x86_64')
        TARGET_ARCH='x86_64'
        TARGET_BIN_PREFIX='x86_64-linux-android'
        ;;
    *)
