...
{
    return DP_msg_draw_dabs_pixel_layer(internal);
}
...
{
    return DP_msg_draw_dabs_mypaint_blend_layer(internal);
}
                             ...
                             int (*get_max_dab_size)(void *))
{
    ...
    if (brush_size_limits[DP_ACCESS_TIER_GUEST] >= 0) {
        int tier_limit =
            brush_size_limits[DP_acl_state_user_tier(acls, user_id)];
        if (tier_limit == 0
