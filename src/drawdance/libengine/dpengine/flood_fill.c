...
typedef struct DP_FloodFillDabContext {
    ...
    DP_Queue queue;
    DP_Rect mask_area;
    DP_Rect mask_on_canvas_area;
    float tolerance_squared;
} DP_FloodFillDabContext;

static bool flood_fill_dab_seed_in_mask(const uint16_t *mask, int diameter,
                                        int offset)
{
    return mask[offset * diameter + offset];
}

static int flood_fill_dab_search_mask_seed_offset(const uint16_t *mask,
                                                  int diameter)
{
    // Some very small MyPaint dabs end up with a blank pixel in their center,
    // and instead end up slightly offset to the top-left, so here we try
    // searching for a non-transparent pixel in that direction.
    int offset = diameter / 2;
    if (diameter < 3) {
        while (offset > 0) {
            if (flood_fill_dab_seed_in_mask(mask, diameter, offset)) {
                return offset;
            }
            --offset;
        }
        ...
        while (offset > 0) {
            ...
            else if (flood_fill_dab_seed_in_mask(mask, diameter, offset + 1)) {
                return offset + 1;
            }
            offset /= 2;
        }
...
static uint16_t mask_get(const uint16_t *mask, DP_Rect area, int x, int y)
{
    DP_ASSERT(DP_rect_contains(area, x, y));
    int bx = x - area.x1;
    int by = y - area.y1;
    return mask[by * DP_rect_width(area) + bx];
}

static bool flood_fill_check_flood(const uint16_t *mask,
                                   DP_LayerContent *flood_lc, DP_Rect mask_area,
                                   float tolerance_squared, int x, int y)
{
    return mask_get(mask, mask_area, x, y) != 0
        && source_should_flood_transparent(
               DP_layer_content_pixel_at(flood_lc, x, y).a, tolerance_squared);
}

static bool flood_fill_dab_should_flood(DP_FloodFillDabContext *c,
                                        unsigned char *b, int x, int y)
{
    if (*b & FLOODED) {
        return *b & SHOULD_FLOOD;
    }
    else if (flood_fill_check_flood(c->mask, c->flood_lc, c->mask_area,
                                    c->tolerance_squared, x, y)) {
        *b |= (unsigned char)(FLOODED | SHOULD_FLOOD);
        return true;
    }
    else {
        *b |= (unsigned char)FLOODED;
        return false;
    }
}

static bool flood_fill_dab_inside(DP_FloodFillDabContext *c, int x, int y)
{
    unsigned char *b;
    return DP_rect_contains(c->mask_on_canvas_area, x, y)
        && !(*(b = buffer_at(c->output, c->mask_area, x, y)) & FILLED)
        && flood_fill_dab_should_flood(c, b, x, y);
}

static void flood_fill_dab_scan(DP_FloodFillDabContext *c, DP_Queue *s, int lx,
                                int rx, int y)
{
    bool added = false;
    for (int x = lx; x <= rx; ++x) {
        if (!flood_fill_dab_inside(c, x, y)) {
            added = false;
        }
        else if (!added) {
            add_seed(s, x, y);
...
static void flood_fill_dab_fill(DP_FloodFillDabContext *c, int x0, int y0)
{
    DP_Queue *s = &c->queue;
    add_seed(s, x0, y0);
    while (s->used != 0) {
        ...
        shift_seed(s, &x, &y);
        int lx = x;
        while (flood_fill_dab_inside(c, lx - 1, y)) {
            *buffer_at(c->output, c->mask_area, lx - 1, y) |= FILLED;
            lx = lx - 1;
        }
        while (flood_fill_dab_inside(c, x, y)) {
            *buffer_at(c->output, c->mask_area, x, y) |= FILLED;
            x = x + 1;
        }
        flood_fill_dab_scan(c, s, lx, x - 1, y + 1);
        flood_fill_dab_scan(c, s, lx, x - 1, y - 1);
    }
}

static bool flood_fill_dab_expand_pixel(DP_FloodFillDabContext *c, int diameter,
                                        int expand, int x0, int y0)
{
    unsigned char *output = c->output;
    int left = DP_max_int(x0 - expand, 0);
    int top = DP_max_int(y0 - expand, 0);
    int right = DP_min_int(x0 + expand, diameter - 1);
    int bottom = DP_min_int(y0 + expand, diameter - 1);
    for (int y = top; y <= bottom; ++y) {
        for (int x = left; x <= right; ++x) {
            if (output[y * diameter + x] & FILLED) {
                return true;
            }
        }
    }
...
static void flood_fill_dab_expand(DP_FloodFillDabContext *c, int diameter,
                                  int expand)
{
    int i = 0;
    for (int y = 0; y < diameter; ++y) {
        for (int x = 0; x < diameter; ++x, ++i) {
            if (flood_fill_dab_expand_pixel(c, diameter, expand, x, y)) {
                c->output[i] |= EXPANDED;
            }
        }
    }
...
static uint16_t flood_fill_dab_flush_at(DP_FloodFillDabContext *c, int i,
                                        unsigned char fill_flag)
{
    // NB: mask and dst can alias! Mask values at indexes < i are clobbered.
    uint16_t m = c->mask[i];
    if (m == 0) {
        return 0;
    }
    else if (c->output[i] & fill_flag) {
        return m;
    }
    else {
        return 0;
                       ...
                       uint16_t *dst, DP_LayerContent *flood_lc)
{
    ...
    DP_Rect mask_area = DP_rect_make(left, top, diameter, diameter);
    if (DP_rect_empty(mask_area)) {
        return false;
    }

    DP_Rect canvas_area = DP_rect_make(0, 0, DP_layer_content_width(flood_lc),
                                       DP_layer_content_height(flood_lc));
    if (DP_rect_empty(canvas_area)) {
        return false;
    }

    DP_Rect mask_on_canvas_area = DP_rect_intersection(mask_area, canvas_area);
    if (DP_rect_empty(mask_on_canvas_area)) {
        return false;
    }

    int seed_offset = flood_fill_dab_search_mask_seed_offset(mask, diameter);
    if (seed_offset < 0) {
        return false;
    }

    int x = DP_clamp_int(left + seed_offset, 0, DP_rect_right(canvas_area));
    int y = DP_clamp_int(top + seed_offset, 0, DP_rect_bottom(canvas_area));
    if (!DP_rect_contains(mask_on_canvas_area, x, y)) {
        return false;
    }
    ...
    DP_queue_init(&c.queue, 1024, sizeof(DP_FillSeed));
    buffer_set(c.output, c.mask_area, x, y, FLOODED | SHOULD_FLOOD);
    flood_fill_dab_fill(&c, x, y);
    DP_queue_dispose(&c.queue);
    ...
    if (expand <= 0) {
        ...
        fill_flag = EXPANDED;
        flood_fill_dab_expand(&c, diameter, expand);
    }

    for (int i = 0; i < count; ++i) {
        dst[i] = flood_fill_dab_flush_at(&c, i, fill_flag);
    }
