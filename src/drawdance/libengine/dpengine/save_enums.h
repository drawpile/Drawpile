// SPDX-License-Identifier: GPL-3.0-or-later
#ifndef DPENGINE_SAVE_ENUMS_H
#define DPENGINE_SAVE_ENUMS_H

#define DP_LOAD_FLAG_SINGLE_THREAD (1u << 0u)
#define DP_LOAD_FLAG_GUESS_PLAYER  (1u << 1u)

#define DP_LOAD_PLAYER_FLAG_COMPATIBLE            (1u << 0u)
#define DP_LOAD_PLAYER_FLAG_MINOR_INCOMPATIBILITY (1u << 1u)
#define DP_LOAD_PLAYER_FLAG_BACKWARD_COMPATIBLE   (1u << 2u)
#define DP_LOAD_PLAYER_FLAG_INCOMPATIBLE          (1u << 3u)
#define DP_LOAD_PLAYER_FLAG_SESSION_TEMPLATE      (1u << 4u)

typedef enum DP_SaveImageType {
    DP_SAVE_IMAGE_UNKNOWN,
    DP_SAVE_IMAGE_ORA,
    DP_SAVE_IMAGE_PNG,
    DP_SAVE_IMAGE_JPEG,
    DP_SAVE_IMAGE_PSD,
    DP_SAVE_IMAGE_WEBP,
    DP_SAVE_IMAGE_PROJECT_CANVAS,
    DP_SAVE_IMAGE_QOI,
    DP_SAVE_IMAGE_PROJECT,
    DP_SAVE_IMAGE_LAST = DP_SAVE_IMAGE_PROJECT,
} DP_SaveImageType;

typedef enum DP_SaveResult {
    DP_SAVE_RESULT_SUCCESS,
    DP_SAVE_RESULT_BAD_ARGUMENTS,
    DP_SAVE_RESULT_BAD_DIMENSIONS,
    DP_SAVE_RESULT_UNKNOWN_FORMAT,
    DP_SAVE_RESULT_FLATTEN_ERROR,
    DP_SAVE_RESULT_OPEN_ERROR,
    DP_SAVE_RESULT_WRITE_ERROR,
    DP_SAVE_RESULT_INTERNAL_ERROR,
    DP_SAVE_RESULT_CANCEL,
} DP_SaveResult;

#endif
