...
camera_alloc(bool transient, bool hidden, int id, uint8_t flags,
             uint8_t interpolation, int fpms, int range_first, int range_last,
             int output_width, int output_height, DP_Rect viewport,
             DP_Text *title_or_null, int count, const DP_CameraEntry *elements,
             int entry_count)
{
...
int DP_camera_fpms(DP_Camera *c)
{
    DP_ASSERT(c);
    DP_ASSERT(DP_atomic_get(&c->refcount) > 0);
    return c->fpms;
...
bool DP_camera_fpms_valid(DP_Camera *c)
{
    DP_ASSERT(c);
    DP_ASSERT(DP_atomic_get(&c->refcount) > 0);
    return c->fpms > 0;
...
DP_TransientCamera *DP_transient_camera_new(DP_Camera *c, int reserve)
{
    ...
    DP_TransientCamera *tc = camera_alloc(
        true, c->hidden, c->id, c->flags, c->interpolation, c->fpms,
        c->range_first, c->range_last, c->output_width, c->output_height,
        c->viewport, c->title, count + reserve, c->elements, count);
    return tc;
}
...
DP_TransientCamera *DP_transient_camera_new_init(int id, int reserve)
{
    ...
    DP_TransientCamera *tc =
        camera_alloc(true, false, id, 0, 0, 0, -1, -1, 0, 0, INVALID_VIEWPORT,
                     NULL, reserve, NULL, 0);
    return tc;
}
...
int DP_transient_camera_fpms(DP_TransientCamera *tc)
{
    DP_ASSERT(tc);
    DP_ASSERT(DP_atomic_get(&tc->refcount) > 0);
    DP_ASSERT(tc->transient);
    return DP_camera_fpms((DP_Camera *)tc);
}

bool DP_transient_camera_fpms_valid(DP_TransientCamera *tc)
{
    DP_ASSERT(tc);
    DP_ASSERT(DP_atomic_get(&tc->refcount) > 0);
    DP_ASSERT(tc->transient);
    return DP_camera_fpms_valid((DP_Camera *)tc);
}
...
void DP_transient_camera_fpms_set(DP_TransientCamera *tc, int fpms)
{
    DP_ASSERT(tc);
    DP_ASSERT(DP_atomic_get(&tc->refcount) > 0);
    DP_ASSERT(tc->transient);
