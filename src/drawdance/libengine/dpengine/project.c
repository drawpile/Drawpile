                         ...
                         bool snapshot_only)
{
    ...
    if (application_id != expected_application_id) {
        ...
        result->error = DP_PROJECT_OPEN_ERROR_HEADER_MISMATCH;
        return false;
    }
...
static DP_ProjectOpenResult project_open(const char *path, unsigned int flags,
                                         bool snapshot_only, bool read_only)
{
    ...
    if (!read_only) {
        ...
        bool setup_ok =
            ...
            && exec_text_stmt(db, "pragma locking_mode", NULL,
                              check_pragma_result,
    ...
    bool init_ok = (!empty || init_header(db, &result, snapshot_only))
                && check_header(db, &result, snapshot_only)
                && (read_only || apply_migrations_in_tx(db, &result));
    if (!init_ok) {
        try_close_db(db);
        return result;
    }
...
{
    return project_open(path, flags, false, false);
}
                           ...
                           bool (*thumb_write)(DP_Image *, DP_Output *))
{
    ...
    DP_ResetImageOptions options = {
        true, true, DP_RESET_IMAGE_COMPRESSION_ZSTD8LE, 256, 256, thumb_write};
...
static void cfs_tile_job(void *element, int thread_index)
{
    ...
    DP_Tile *t = DP_tile_new_from_compressed_zstd8le(
        &c->zstd_contexts[thread_index], 0, job->data, job->header.size,
        c->pixel_buffers[thread_index]);
                           ...
                           sqlite3_stmt *stmt, int *in_out_increments)
{
    ...
    if (title_length > DP_MSG_LAYER_CREATE_TITLE_MAX_LEN) {
        DP_warn("Layer title length %zu out of bounds", title_length);
        title_length = DP_MSG_LAYER_CREATE_TITLE_MAX_LEN;
    }
...
static bool cfs_read_annotations(DP_ProjectCanvasFromSnapshotContext *c)
{
    ...
    while (ps_exec_step(prj, stmt, &error)) {
        ...
        if (annotation_id <= 0 || annotation_id > UINT16_MAX) {
        ...
        DP_transient_annotation_protect_set(
            ta, flags & DP_PROJECT_SNAPSHOT_ANNOTATION_FLAG_PROTECTED);
                                      ...
                                      int count, DP_TransientKeyFrame **ptkf)
{
    ...
    while (ps_exec_step(prj, layer_stmt, &error)) {
        ...
        if (layer_id <= 0 || layer_id > UINT16_MAX) {
            DP_warn("Layer id %d in frame %d of track %d out of bounds",
                    layer_id, frame_index, track_index);
            continue;
        }
                              ...
                              DP_TransientTrack **tracks)
{
    ...
    while (ps_exec_step(prj, track_stmt, &error)) {
        ...
        if (track_id <= 0 || track_id > UINT16_MAX) {
                                                ...
                                                long long snapshot_id)
{
    ...
    c.pixel_buffers =
        DP_malloc(sizeof(*c.pixel_buffers) * DP_int_to_size(thread_count));
    c.zstd_contexts =
        DP_malloc(sizeof(*c.zstd_contexts) * DP_int_to_size(thread_count));
    for (int i = 0; i < thread_count; ++i) {
        c.pixel_buffers[i] = DP_malloc(sizeof(DP_Pixel8) * DP_TILE_LENGTH);
                           ...
                           bool (*thumb_write)(DP_Image *, DP_Output *))
{
    ...
    DP_ProjectOpenResult open_result =
        project_open(path, DP_PROJECT_OPEN_TRUNCATE, true, false);
                           ...
                           DP_CanvasState **out_cs)
{
    ...
    DP_ProjectOpenResult open_result =
        project_open(path, DP_PROJECT_OPEN_EXISTING, true, true);
    DP_Project *prj = open_result.project;
    if (!prj) {
        return open_result.error;
    }
