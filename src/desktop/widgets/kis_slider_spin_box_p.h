...
public:
    ...
    void updateWidgetRangeToggleTooltip()
    {
        m_widgetRangeToggle->setToolTip(
            tr("Toggle between full range and subrange.\nFull range: [%1, %2]\nSubrange: [%3, %4]")
                .arg(QString::number(m_q->minimum()))
                .arg(QString::number(m_q->maximum()))
                .arg(QString::number(m_softMinimum))
                .arg(QString::number(m_softMaximum))
        );
    }
    ...
    bool qContextMenuEvent(QContextMenuEvent *e)
    {
        ...
        if (isEditModeActive()) {
            selectAllAction = new QAction(tr("&Select All"), menu);
        ...
        QAction *stepDown = menu->addAction(tr("Step &down"));
    ...
    bool eventFilter(QObject * o, QEvent * e) override
    {
        ...
        if (o == m_q) {
            switch (e->type()) {
                ...
                // Drawpile patch: I guess we use shortcuts differently from
                // Krita. We have to accept the appropriate shortcut override
                // events so that we get the key presses we're interested in.
                case QEvent::ShortcutOverride : return qShortcutOverrideEvent(static_cast<QKeyEvent*>(e));
