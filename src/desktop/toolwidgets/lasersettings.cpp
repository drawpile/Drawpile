...
QWidget *LaserPointerSettings::createUiWidget(QWidget *parent)
{
	...
	connect(
		m_ui->persistence, QOverload<int>::of(&KisSliderSpinBox::valueChanged),
		this, &LaserPointerSettings::pushSettings);
	connect(
...
void LaserPointerSettings::quickAdjust1(qreal adjustment, bool wheel)
{
	quickAdjustOn(m_ui->persistence, adjustment, wheel, m_quickAdjust1);
}

void LaserPointerSettings::stepAdjust1(bool increase)
{
	KisSliderSpinBox *persistence = m_ui->persistence;
