	...
	: libclient::settings::Settings::Settings(parent)
{
	...
	qRegisterMetaTypeStreamOperators<widgets::CanvasView::BrushCursor>(
		"widgets::CanvasView::BrushCursor");
	qRegisterMetaTypeStreamOperators<color_widgets::ColorWheel::AngleEnum>(
		"color_widgets::ColorWheel::AngleEnum");
	qRegisterMetaTypeStreamOperators<color_widgets::ColorWheel::ColorSpaceEnum>(
		"color_widgets::ColorWheel::ColorSpaceEnum");
	qRegisterMetaTypeStreamOperators<color_widgets::ColorWheel::ShapeEnum>(
		"color_widgets::ColorWheel::ShapeEnum");
...
{
	return libclient::settings::getEnumReplacedByInt<
		color_widgets::ColorWheel::AngleEnum>(meta, settings);
}
}
...
{
	return libclient::settings::getEnumReplacedByInt<
		color_widgets::ColorWheel::ShapeEnum>(meta, settings);
}
}
...
{
	return libclient::settings::getEnumReplacedByInt<
		color_widgets::ColorWheel::ColorSpaceEnum>(meta, settings);
}
}
...
int bounded(const QVariant &value)
{
	...
	return ok ? qBound(20, i, 1000) : config::Config::defaultDebounceDelayMs();
}
...
QVariant get(const SettingMeta &meta, QSettings &settings)
{
	...
	if(hostRemoteKey.has_value() &&
	   !settings.value(hostRemoteKey->key).toBool()) {
		return 2;
	}
...
QVariant get(const SettingMeta &meta, QSettings &settings)
{
	return libclient::settings::getEnumReplacedByInt<tools::Tool::Type>(
		meta, settings);
}
}
...
QVariant get(const SettingMeta &meta, QSettings &settings)
{
	if(const auto key = findKey(settings, meta.baseKey, meta.version)) {
		return any::getGroup(settings, key->key);
				  ...
				  settings, "onionskins/frame1", SettingMeta::Version::V0)) {
		Settings::OnionSkinsFramesType frames;
		const auto size = settings.value("onionskins/framecount").toInt();
		for(auto i = 0; i < size; ++i) {
			frames.insert(
				i, settings.value(QStringLiteral("frame%1").arg(i)).toInt());
		}
...
void set(const SettingMeta &meta, QSettings &settings, QVariant value)
{
	...
	if(findKey(settings, "onionskins/frame1", SettingMeta::Version::V0)) {
		...
		for(auto entry = frames.cbegin(); entry != frames.cend(); ++entry) {
			settings.setValue(
				QStringLiteral("frame%1").arg(entry.key()), entry.value());
		}
	}
}
}
...
QVariant get(const SettingMeta &meta, QSettings &settings)
{
	...
	std::optional<libclient::settings::FoundKey> touchDrawKey =
		findKey(settings, "settings/input/touchdraw", SettingMeta::Version::V0);
	...
	if(touchScrollKey.has_value() &&
	   settings.value(touchScrollKey->key).toBool()) {
...
{
	QVariant value =
		libclient::settings::getEnumReplacedByInt<tabletinput::Mode>(
			meta, settings);
	switch(value.toInt()) {
