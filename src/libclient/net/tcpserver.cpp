	...
	, m_supportsPersistence(false)
{
	...
	m_msgqueue = new MessageQueue(m_socket, true, this);
	...
	connect(
		m_msgqueue, &MessageQueue::messageAvailable, this,
		&TcpServer::handleMessage);
	connect(
		m_msgqueue, &MessageQueue::bytesReceived, this,
		&TcpServer::bytesReceived);
	connect(m_msgqueue, &MessageQueue::bytesSent, this, &TcpServer::bytesSent);
	connect(
		m_msgqueue, &MessageQueue::badData, this, &TcpServer::handleBadData);
	connect(m_msgqueue, &MessageQueue::pingPong, this, &TcpServer::lagMeasured);
	connect(
		m_msgqueue, &MessageQueue::gracefulDisconnect, this,
		&TcpServer::gracefullyDisconnecting);
}
...
void TcpServer::login(LoginHandler *login)
{
	...
	m_socket->connectToHost(
		login->url().host(), login->url().port(cmake_config::proto::port()));
}

void TcpServer::logout()
{
	m_localDisconnect = true;
	m_msgqueue->sendDisconnect(
...
void TcpServer::sendMessage(const net::Message &msg)
{
	m_msgqueue->send(msg);
}

void TcpServer::sendMessages(int count, const net::Message *msgs)
{
