...
ColorPickRenderer::ColorPickRenderer(
...
bool ColorPickRenderer::updateColor(const QColor &color)
{
...
bool ColorPickRenderer::updateComparisonColor(const QColor &comparisonColor)
{
	QColor c = sanitizeColor(comparisonColor);
	if(c != m_comparisonColor) {
		m_comparisonColor = c;
		m_cache = QPixmap();
		return true;
	} else {
...
void ColorPickRenderer::paint(
	QPainter *painter, const QRect &rect, const QColor &baseColor)
{
...
bool ColorPickRenderer::shouldShow(int source, int visibility)
{
	if(source == int(tools::ColorPickSource::Adjust)) {
		return true;
	} else {
		switch(visibility) {
		case int(view::SamplingRingVisibility::Never):
			return false;
		case int(view::SamplingRingVisibility::TouchOnly):
...
int ColorPickRenderer::defaultVisibility()
{
	return int(view::SamplingRingVisibility::Always);
}

const QPainterPath &
ColorPickRenderer::getPath(qreal penWidth, const QRectF &cacheRect)
{
...
QColor ColorPickRenderer::sanitizeColor(const QColor &color)
{
