...
DP_FloodFillResult CanvasState::floodFill(
    ...
    bool sampleMerged, int sizeLimit, int gap, int expand, int featherRadius,
    const QAtomicInt &cancel, QImage &outImg, int &outX, int &outY) const
{
    ...
    DP_FloodFillResult result = DP_flood_fill(
        ...
        gap, expand, featherRadius, &img, &outX, &outY, shouldCancelFloodFill,
...
drawdance::CanvasState CanvasState::makeBackwardCompatible() const
{
    ...
    DP_TransientLayerList *tll = DP_transient_canvas_state_transient_layers(tcs, 0);
    DP_TransientLayerPropsList *tlpl = DP_transient_canvas_state_transient_layer_props(tcs, 0);
    ...
    return drawdance::CanvasState::noinc(DP_transient_canvas_state_persist(tcs));
}
