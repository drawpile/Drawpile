...
{
	if(m_canvas) {
		m_canvas->discardProjectRecording();
	}
	delete m_canvas;
	...
	bool autoRecord)
{
	...
	if(autoRecord) {
		m_canvas->startProjectRecording(DP_PROJECT_SOURCE_BLANK);
	}
	...
	DP_SaveImageType type, bool dirty, bool autoRecord)
{
	...
	if(autoRecord) {
		m_canvas->startProjectRecording(DP_PROJECT_SOURCE_FILE);
		if(!path.isEmpty()) {
...
void Document::onServerLogin(const net::LoggedInParams &params)
{
	...
	if(params.join && m_projectRecordOnConnect) {
		m_canvas->startProjectRecording(DP_PROJECT_SOURCE_SESSION, false);
	}
