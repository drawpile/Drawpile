...
void CanvasModel::handleCommands(int count, const net::Message *msgs)
{
	...
	if(m_paintengine->receiveMessages(false, count, msgs) != 0) {
		...
		bool projectThumbnailTimerReady =
			m_projectHandler && m_projectHandler->isThumbnailTimerReady();
...
bool CanvasModel::startProjectRecording(int sourceType, bool requestThumbnail)
{
	if(m_projectHandler) {
	...
	m_projectHandler = new project::ProjectHandler(this);
	connect(
