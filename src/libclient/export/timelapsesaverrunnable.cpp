...
void TimelapseSaverRunnable::run()
{
	...
	bool saveOk = saveVideo(errorMessage);
	...
	if(shouldStop()) {
		...
		if(!logoImage.isNull()) {
			if(logoImage.format() == QImage::Format_ARGB32_Premultiplied) {
				...
				scaleLogoImage(
					logoImage.convertedTo(QImage::Format_ARGB32_Premultiplied));
			}
		}

		QImage finalImage = toOutputImage(toFlatImage(m_parent->m_canvasState));
		...
		result = DP_project_playback_play(
			m_parent->m_projectPlayback, m_dc, framerate,
	...
	if(img.isNull() || img.size().isEmpty()) {
		...
		if(logoRect.isEmpty()) {
			...
			DP_Image *scaledImg = DP_image_scale_pixels(
				...
				reinterpret_cast<const DP_Pixel8 *>(img.constBits()), m_dc,
...
DP_Image *TimelapseSaverRunnable::PlaybackRunnable::toFlatImage(
	const drawdance::CanvasState &canvasState)
{
	if(canvasState.isNull()) {
		return nullptr;
	} else {
		return DP_canvas_state_to_flat_image(
			canvasState.get(), DP_FLAT_IMAGE_RENDER_FLAGS, NULL, NULL);
	}
}

QImage TimelapseSaverRunnable::PlaybackRunnable::toOutputImage(DP_Image *img)
{
	if(!img) {
		return QImage();
	}
	...
	if(width != scaledWidth || height != scaledHeight) {
		...
		DP_Image *scaledImg = DP_image_scale(
			img, m_dc, targetWidth, targetHeight, m_parent->m_interpolation);
		DP_image_free(img);
		if(!scaledImg) {
			return QImage();
		}
	...
	int instances, const drawdance::CanvasState &canvasState)
{
	...
	if(instances > 0) {
		QImage img = toOutputImage(toFlatImage(canvasState));
		if(img.isNull()) {
			qCWarning(
	...
	void *user, int instances, DP_CanvasState *cs)
{
	return static_cast<TimelapseSaverRunnable::PlaybackRunnable *>(user)
		->handle(instances, drawdance::CanvasState::noinc(cs));
}
	...
	DP_Project *prj, QString &outErrorMessage) const
{
	...
	int result = DP_project_playback_measure(pb, m_maxDeltaSeconds);
	if(result != 0) {
		qCWarning(
			lcDpTimelapseSaverRunnable,
