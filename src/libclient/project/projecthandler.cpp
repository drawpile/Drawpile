	...
	QString *outError)
{
	...
	m_thumbnailTimer->setInterval(5 * 60 * 1000);
	connect(
...
void ProjectHandler::startThumbnailTimer()
{
	if(isThumbnailTimerReady()) {
		qCDebug(lcDpProjectWorker, "Starting thumbnail timer");
...
void ProjectHandler::saveToProjectFile(const QString &path) {}
...
void ProjectHandler::handleEvent(const DP_ProjectWorkerEvent *event)
{
	...
	case DP_PROJECT_WORKER_EVENT_MESSAGE_RECORD_ERROR:
		...
		return;
	case DP_PROJECT_WORKER_EVENT_SNAPSHOT_MAKE_ERROR:
		emitError(
			//: %1 is an error code, %2 is a more detailed error message.
...
bool ProjectHandler::writeThumbnail(DP_Image *thumb)
{
	...
	if(!DP_image_write_project_thumbnail(NULL, thumb, output)) {
